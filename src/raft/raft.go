...
type Raft struct {
	...
	voteFor         int          // 当前任期内收到选票的candidateId，如果没有则为空
	logs            []LogEntry   // 日志条目；每个条目包含了用于状态机的命令，以及领导人接收到该条目时的任期
	commitIndex     int          // 已知已提交的最高的日志条目的索引
	lastApplied     int          // 已知被应用到状态机的最高的日志条目的索引
	nextIndexs      []int        // 对于每一台服务器，发送到该服务器的下一个日志条目的索引
	matchIndexs     []int        // 对于每一台服务器，已知的复制到该服务器的最高日志条目的索引
	myStatus        Status       // Server的状态

	timer           *time.Ticker //timer
	voteTimeout     time.Duration // 选举超时时间，选举超时时间是会变动的，所以定义在Raft结构体里
	applyChan       chan ApplyMsg // 消息channel

	//2D
	lastIncludeIndex    int      // snapshot保存的最后的log的index
	lastIncludeTerm     int      // snapshot保存的最后的log的term
	snapshotCmd         []byte

}

type LogEntry struct {
	Term   int   // LogEntry里记录有任期
